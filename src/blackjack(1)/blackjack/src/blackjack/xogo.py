...
class Xogo:
    ...
    def xogar(self):
        ...
        while any((xogador.podePedir() for xogador in self._xogadores)):
            max_valor = max(list(xogador.puntos for xogador in self._xogadores))
            for xogador in self._xogadores:
                if xogador.podePedir():
                    print(f"Xogador {xogador.nome}")
                    decision = xogador.decidir(max_valor)
        ...
        if ganhador:
            ...
            print(list((xogador.puntos for xogador in self._xogadores)))
        else:
...
class Xogador:
    ...
    def decidir(self,max_valor):
        return self.estratexia.decidir(self._mano,max_valor)
...
class EstratexiaXogo():
    ...
    def decidir(self, mano):
        pass


class EstratexiaXogoHumano(EstratexiaXogo):

    def decidir(self, mano,max_valor_mesa):
...
class EstratexiaDoNumero15(EstratexiaXogo):

    def decidir(self, mano,max_valor_mesa):
        print(f"Agora tes {mano.puntos}")
        if mano.puntos < 15:
            return 'pedir'
        elif mano.puntos >= 15 and mano.puntos < max_valor_mesa:
            return 'pedir'
        else:
            return 'plantarse'
